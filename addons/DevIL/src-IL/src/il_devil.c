...
ILboolean ILAPIENTRY ilBlit(ILuint Source, ILint DestX,  ILint DestY,   ILint DestZ, 
   ...
   return ilBlitAlpha(Source, DestX, DestY, DestZ,
	   SrcX, SrcY, SrcZ, Width, Height, Depth, IL_TRUE);
}
//@NEXT DestX,DestY,DestZ must be set to ILuint
ILboolean ILAPIENTRY ilBlitAlpha(ILuint Source, ILint DestX,  ILint DestY,   ILint DestZ, 
                                           ILuint SrcX,  ILuint SrcY,   ILuint SrcZ,
                                           ILuint Width, ILuint Height, ILuint Depth, ILboolean BlendAlpha) {
	...
	if( BlendAlpha == IL_TRUE && 
		( Src->Format == IL_RGBA || Src->Format == IL_BGRA || Src->Format == IL_LUMINANCE_ALPHA )) {
